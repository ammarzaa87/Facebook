...
<input type="hidden" id="u_id" name="u_id" value="<?php echo $u_id?>">
	...
	async function showfriends(){
				const response = await fetch('http://localhost/facebook/php/showfriends.php?u_id='+$('#u_id').val());
				if(!response.ok){
		...
		for (var i = 0; i < data.length; i++){
			var row = `<div class="row member-row">
						   ...
						   <div class="col-md-3">
							...
							<div class="col-md-3">
							  <p><a id="acc" data-id=${data[i].id} class="btn btn-success btn-block"><i class="fa fa-users"></i>Friends</a></p>
							</div>
						
							<div class="col-md-3">
							  <p><a  id="remv" data-id=${data[i].id} class="btn btn-primary btn-block"><i class="fa fa-remove"></i>Remove Friend</a></p>
							</div>
							<div class="col-md-3">
							  <p><a id="block" data-id=${data[i].id} class="btn btn-danger btn-block"><i class="fa fa-ban"></i>Block Friend</a></p>
							</div>
...
$(document).on('click','#block',function(){
	...
	success: function(dataResult){
					...
					if(dataResult.statusCode==200){
						...
						var message = "You Blocked "+fname+" "+lname+", you are not friends anymore";
						$('#myTable').empty();
						$( document ).ready(function() {
								showfriends().then(results => {
									console.log(results);
